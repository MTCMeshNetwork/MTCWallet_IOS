...
@property (nonatomic, strong) Payment *blePayment;
...
@property (nonatomic, readonly) UIView *blescanButton;
...
- (void)updateBlescan: (NSString *)address {
    BOOL enabled = !!address.length;
    
    __weak ScannerConfigController *weakSelf = self;
    void (^animate)() = ^() {
        weakSelf.blescanButton.alpha = (enabled ? 1.0f: 0.0f);
    };
    [UIView animateWithDuration:0.5f animations:animate];
}
...
//        }
        ...
        if (name) {
            ...
            addressStr = _foundName;
        } else if (address) {
            self.navigationItem.prompt = shortAddress(_foundAddress);
            addressStr = address.checksumAddress;
...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.tintColor = [UIColor commonBackgroundColor];
    self.navigationController.navigationBar.backgroundColor = [UIColor commonBackgroundColor];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(dismissWithNil)];
...
//    self.nextTitle = @"Next";
    ...
    [_photosView setBackgroundColor:[UIColor commonBackgroundColor]];
    ...
    _blescanButton = [self addButtonTitle:ICON_NAME_AIRPLANE action:@selector(didTapBlescan:)];
    _blescanButton.alpha = 0.0f;
    _blescanButton.center = CGPointMake(30.0f + 15.0f, size.height - 260.0f - 30.0f - 15.0f);
...
- (void)didTapBlescan: (UIButton *)button {
    [self setAddress:self.blePayment.address name:nil amount:nil promptType:PromptTypeFoundPasteboard];
}
...
- (void)viewDidAppear:(BOOL)animated {
     ...
     */
//    __weak ScannerConfigController *weakSelf = self;
//    [BLEManager scanAdrress:^(NSString *address) {
//        [weakSelf updateBlescan:address];
//        if(address.length) {
//            weakSelf.blePayment = [Payment paymentWithURI:address];
//            [BLEManager scanAdrress:nil];
//        }
//    }];
}
